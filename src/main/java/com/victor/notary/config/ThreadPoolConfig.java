...
@Component
public class ThreadPoolConfig implements BeanFactoryAware {
    //用于从IOC里取对象
    ...
    /**
     * 用于储存在队列中的请求,防止重复提交,在真实场景中，可用redis代替 验证重复
     */
    Map<String, Object> cacheMap = new ConcurrentHashMap<>();


    /**
     * 转账的缓冲队列,当线程池满了，则将转账请求存入到此缓冲队列
     */
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            // 交易加入到缓冲队列
            msgQueue.offer(((ThreadConfig) r).getAcceptStr());
            //System.out.println("系统任务太忙了,把此交易交给(调度线程池)逐一处理，交易号：" + ((ThreadConfig) r).getAcceptStr());
        }
    };
    ...
    public void addOrders(String orderId){
        ...
        if (cacheMap.get(orderId) == null) {
            ...
            ThreadConfig businessThread = new ThreadConfig(orderId);
